...
class ExerciseRunner:
    ...
    def do_net_cli(self):
        ...
        #CLI(self.net)
        while(True):
            #f=open("~/tutorials/exercises/big/myfile", "a+")
            f=open("/home/p4/tutorials/exercises/big/myfile", "r+")
            #
            print("aaa")
            cc=f.read()
            ...
            list1=cc.split(" ")
            if (len(list1)==4):
                a,b=self.net.get(list1[1],list1[2])

                self.net.iperf(hosts=(a,b),l4Type="UDP",udpBw=list1[3])
            if(cc=="pingall"):
                self.net.pingAll()
            elif(cc=="iperf h1 h2"):
                h1,h2,h3=self.net.get("h1","h2","h3")
